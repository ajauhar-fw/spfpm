...
class FXnum(object):
    ...
    def __int__(self):
        """Cast to integer"""
        if self.scaledval >= 0:
            return int(round(self.scaledval // self.family.scale))
        else:
            return int(round((self.scaledval + self.family.scale - 1) // self.family.scale))
    ...
    def toDecimalString(self, precision=None, round10=False):
        ...
        if precision is None or not isinstance(precision, int):
            precision = int(round((3 + self.family.fraction_bits) / 3.32))
